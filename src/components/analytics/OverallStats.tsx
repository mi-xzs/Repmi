...
import { computeOverall, fmtDuration, fmtVolume, getCurrentStreak, getLongestStreak, muscleForExercise } from "../../utils/analyticsHelpers";
...
function computeTopMuscles(sessions: WorkoutSession[]): { name: string; sets: number; kg: number; reps: number }[] {
...
const OverallStats: React.FC<Props> = ({
  ...
  const data = useMemo(
    () => computeOverall(workouts, allSessions),
  ...
  const consistency    = useMemo(() => getConsistencyScore(allSessions, weeklyTarget), [allSessions, weeklyTarget]);
