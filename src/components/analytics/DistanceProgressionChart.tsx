...
const getSessionDistance = (entry: HeatmapEntry): number =>
  entry.allSets
    .filter((s) => s.label !== "W")
    .reduce((sum, s) => sum + (s.meters ?? 0), 0);
...
export default function DistanceProgressionChart({ data, exerciseName = "Exercise", title }: Props) {
  const { accent } = useAccent();
  const chartWidth = Math.min(Dimensions.get("window").width - 32, 345);

  const stats = useMemo(() => {
    const valid = data.filter((e) => getSessionDistance(e) > 0);
    if (!valid.length) return null;
    const latest   = valid[valid.length - 1];
    const previous = valid[valid.length - 2];
    const totalDistance = getSessionDistance(latest);
    const diff = previous ? totalDistance - getSessionDistance(previous) : null;
    let allTimePR = 0;
    for (const e of data) {
      const t = getSessionDistance(e);
      if (t > allTimePR) allTimePR = t;
    }
    return { totalDistance, diff, allTimePR };
  }, [data]);

  const recent  = data.slice(-MAX_BARS);
  const volumes = recent.map(getSessionDistance);
  ...
  const yLabels = [0, 1, 2, 3].map((i) => ({
    y:     PAD_T + (i / 3) * innerH,
    label: formatDistance(Math.round(maxVol * (1 - i / 3))) || "0m",
  }));

  const latest      = data[data.length - 1];
  const setBars = (latest?.allSets ?? [])
    .filter((s) => s.label !== "W")
    .map((s) => ({ label: s.label, meters: s.meters ?? 0 }))
    .filter((s) => s.meters > 0);
  const maxSetMeters = Math.max(...setBars.map((s) => s.meters), 1);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.sectionLabel}>distance progression</Text>
        ...
        {stats ? (
          <>
            <View style={styles.statCard}>
              <Text style={styles.statValue}>{formatDistance(stats.totalDistance) || "0m"}</Text>
              <Text style={styles.statLabel}>total distance</Text>
            </View>
            <View style={styles.statCard}>
              {stats.diff !== null ? (
                <Text style={[styles.statValue, stats.diff >= 0 ? styles.positive : styles.negative]}>
                  {stats.diff >= 0 ? "+" : "-"}{formatDistance(Math.abs(stats.diff)) || "0m"}
                </Text>
              ) : (
            ...
            <View style={styles.statCard}>
              <Text style={styles.statValue}>{formatDistance(stats.allTimePR) || "0m"}</Text>
              <Text style={styles.statLabel}>all-time PR</Text>
            </View>
          </>
        ) : (
          ["total distance", "vs previous", "all-time PR"].map((label) => (
      ...
      {recent.length > 0 ? (
        <>
          <Text style={styles.subLabel}>Session Distance</Text>
      ...
      {setBars.length >= 2 && (
        <>
          ...
          <View style={styles.setBarsContainer}>
            {setBars.map((set, i) => (
              <View key={`sb-${i}`} style={styles.setBarRow}>
                <Text style={styles.setBarLabel}>{set.label}</Text>
                <AnimatedBar
                  percent={Math.round((set.meters / maxSetMeters) * 100)}
                  delay={Math.min(i, 6) * 60}
                  trackStyle={styles.setBarTrack}
                  fillStyle={[
                    styles.setBarFill,
                    {
                ...
                />
                <Text style={styles.setBarValue}>{formatDistance(set.meters) || "0m"}</Text>
              </View>
            ))}
          </View>
        </>
