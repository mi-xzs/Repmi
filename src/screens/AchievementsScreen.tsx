...
import { dayKey, getCurrentStreak, getLongestStreak, muscleForExercise } from '../utils/analyticsHelpers';
...
const RARITY_CRIMSON: Record<Rarity, RarityToken> = {
  ...
  legendary: {
    key: 'legendary', label: 'LEGENDARY', stars: 5,
    color: '#FF0040',
            ...
            <View style={cardStyles.cardRow}>
              ...
              >
                ...
                <View style={cardStyles.coinRow}>
                  ...
                  <Text style={[cardStyles.coinText, { color: colors.fav }]}>+{getCoinsForAchievement(achievement.xp)}</Text>
                </View>
              </View>
            </View>
          </View>
          ...
          <View style={[modalStyles.card, { borderColor: tier.borderActive }]}>
            ...
            <View style={[modalStyles.modalReward, { backgroundColor: tier.color + '18', borderColor: tier.color + '66' }]}>
              ...
              <View style={modalStyles.modalRewardCoinRow}>
                ...
                <Text style={[modalStyles.modalRewardCoin, { color: colors.fav }]}>+{getCoinsForAchievement(achievement.xp)} COINS</Text>
              </View>
            </View>
...
function SlotAvatar({
  ...
  const ringPulse = useAnimatedStyle(() => ({
    ...
    transform: [{ scale: isFirst ? interpolate(shimmer.value, [0, 1], [0.96, 1.07], Extrapolation.CLAMP) : 1 }],
  }));
  ...
  return (
    <View style={[topShowcaseStyles.tileWrap, { height: ringSize }]}>
      ...
      {isFirst && (
        <View pointerEvents="none" style={[topShowcaseStyles.spotlightColumn, { height: ringSize + 28 }]}>
          <LinearGradient
  ...
  return (
    <View style={topShowcaseStyles.identity}>
      ...
      >
        <Text
          style={[
            topShowcaseStyles.ribbonText,
            { color: tier.color, opacity: isFirst ? 1 : 0.85 },
          ]}
          numberOfLines={1}
        >
          {tier.label}
      ...
      <Text
        style={[
          topShowcaseStyles.rankLabel,
          isFirst && { color: tier.color },
        ]}
      >
        {ordinal}
      </Text>
      ...
      {isYou && (
        <View style={[topShowcaseStyles.youPill, { backgroundColor: accentSubtle, borderColor: accent + '55' }]}>
...
function SlotXP({
  ...
  if (!isFirst) {
    return (
      <Text style={[topShowcaseStyles.xp, { color: colors.titleText }]}>
        {formatXP(target)}
      </Text>
    );
  }
  ...
  return (
    <AnimatedTextInput
      ...
      style={[topShowcaseStyles.xp, topShowcaseStyles.xpFirst, topShowcaseStyles.xpInput, { color: tier.color }]}
      accessible
...
function TopShowcaseSlot({ entry, avatarUrl, displayName, delay }: { entry: ShowcaseEntry; avatarUrl?: string | null; displayName?: string | null; delay: number }) {
  ...
  const wrapperEntering = isFirst
    ? undefined
    : FadeInDown.delay(delay).duration(340).easing(Easing.out(Easing.back(1.2)));
  ...
  return (
    <View style={topShowcaseStyles.sectionHeaderWrap}>
      <View style={leaderboardStyles.sectionHeader}>
        ...
        <Reanimated.View style={[leaderboardStyles.sectionDot, { backgroundColor: themedRarity.legendary.color }, dotPulseStyle]} />
      ...
      >
        <Defs>
          ...
          <RadialGradient id="backdrop-core-glow" cx="0.5" cy="0.55" rx="0.7" ry="0.6">
            ...
            <Stop offset="0.5" stopColor={accent} stopOpacity={0.10} />
        ...
        <G stroke="rgba(255,255,255,0.045)" strokeWidth={1} fill="none">
          <Path d={`M 0 ${height * 0.18} Q ${SANDBOX_BACKDROP_W * 0.35} ${height * 0.10}, ${SANDBOX_BACKDROP_W * 0.7} ${height * 0.20} T ${SANDBOX_BACKDROP_W} ${height * 0.16}`} />
          <Path d={`M 0 ${height * 0.32} Q ${SANDBOX_BACKDROP_W * 0.25} ${height * 0.40}, ${SANDBOX_BACKDROP_W * 0.55} ${height * 0.30} T ${SANDBOX_BACKDROP_W} ${height * 0.34}`} />
          <Path d={`M 0 ${height * 0.52} Q ${SANDBOX_BACKDROP_W * 0.45} ${height * 0.46}, ${SANDBOX_BACKDROP_W * 0.80} ${height * 0.54} T ${SANDBOX_BACKDROP_W} ${height * 0.50}`} />
          <Path d={`M 0 ${height * 0.72} Q ${SANDBOX_BACKDROP_W * 0.30} ${height * 0.80}, ${SANDBOX_BACKDROP_W * 0.65} ${height * 0.70} T ${SANDBOX_BACKDROP_W} ${height * 0.74}`} />
          <Path d={`M 0 ${height * 0.88} Q ${SANDBOX_BACKDROP_W * 0.40} ${height * 0.82}, ${SANDBOX_BACKDROP_W * 0.75} ${height * 0.90} T ${SANDBOX_BACKDROP_W} ${height * 0.86}`} />
        </G>
        ...
        <G fill={accent} fillOpacity={0.4}>
          ...
          <Circle cx={SANDBOX_BACKDROP_W * 0.30} cy={height * 0.18} r={1.0} />
...
function PodiumPillar({
  ...
  const pulseStyle = useAnimatedStyle(() => ({
    transform: [
      { scale: interpolate(pulse.value, [0, 1], [1, 1.08], Extrapolation.CLAMP) },
    ],
  }));
...
function ProfilePreviewModal({
  ...
  const fillPct =
    data.totalEntries > 0
      ? Math.max(
          5,
          Math.round((1 - (data.rank - 1) / data.totalEntries) * 100),
        )
      : 0;
  ...
  const topPct =
    data.totalEntries > 0
      ? Math.ceil((data.rank / data.totalEntries) * 100)
      : 0;
            ...
            >
              ...
              <View style={previewStyles.headerRow}>
                ...
                <View style={previewStyles.identity}>
                  <Text
                    style={previewStyles.displayName}
                    numberOfLines={1}
                  >
                    {data.displayName ?? data.name}
                    {data.isYou && (
                      <Text style={previewStyles.youTag}> · YOU</Text>
                    )}
                  </Text>
                  <Text
                    style={[previewStyles.title, { color: r.color }]}
                    numberOfLines={1}
                  >
                    {data.title}
                  </Text>
                  <View style={previewStyles.rankRow}>
                    ...
                    <Text
                      style={[previewStyles.delta, { color: deltaColor }]}
                    >
                      {deltaGlyph}
              ...
              <View style={previewStyles.statGrid}>
                ...
                <View
                  style={[
                    previewStyles.statCell,
                    previewStyles.statCellRight,
                  ]}
                >
                  <Text style={previewStyles.statLabel}>WEEKLY XP</Text>
                  <Text style={previewStyles.statValue}>
                    {formatXP(data.weeklyXP)}
                  </Text>
                </View>
              </View>
              ...
              <View style={previewStyles.percentileWrap}>
                ...
                <Text style={previewStyles.percentileText}>
                  {data.rank === 1
                    ? 'LEADING THE FIELD'
                    : `TOP ${topPct}% OF ${data.totalEntries}`}
                </Text>
              </View>
              ...
              >
                <Text style={[previewStyles.ctaText, { color: r.color }]}>
                  SEE PROFILE
                </Text>
                <Feather
                  name="arrow-right"
            ...
            <Reanimated.View style={[addFriendStyles.card, cardStyle]}>
              ...
              <View style={addFriendStyles.headerRow}>
                ...
                <Pressable
                  onPress={onClose}
                  hitSlop={10}
                  style={addFriendStyles.closeBtn}
                >
                ...
                {loading ? (
                  ...
                  <View style={addFriendStyles.statusWrap}>
                    <Feather
                      ...
                      color="rgba(255,255,255,0.3)"
                    />
                    <Text style={addFriendStyles.statusText}>
                      ...
                      return (
                        <View key={r.id} style={addFriendStyles.row}>
                          ...
                          >
                            ...
                            />
                            <Text
                              style={addFriendStyles.username}
                              numberOfLines={1}
                            >
                              {r.username}
                            </Text>
                          ...
                          <Pressable
                            ...
                            style={({ pressed }) => [
                              ...
                              following
                                ...
                                : [addFriendStyles.followBtnIdle, { borderColor: accent + '66', backgroundColor: accent + '14' }],
                              pressed && { opacity: 0.7 },
                              pending && { opacity: 0.5 },
                            ]}
                          >
...
function ChaseRow({ entry, delta, index, isLast, onPress, onLongPress }: {
  entry: ChaseEntry;
  delta: number;
  index: number;
  isLast?: boolean;
  ...
  return (
    ...
    >
      ...
      >
        <Reanimated.View
          style={[chaseStyles.row, !isLast && chaseStyles.rowDivider, pressStyle]}
        >
          <Reanimated.View
            style={[
              chaseStyles.rankPip,
...
function StandingRankBlock({ rank, tier }: { rank: number; tier: RarityToken }) {
  return (
    <View style={[youStandingStyles.rankBlock, { borderColor: tier.color + '55', backgroundColor: tier.color + '12' }]}>
  ...
  return (
    <View style={youStandingStyles.info}>
      ...
      <View style={youStandingStyles.youLine}>
        ...
        <Text style={youStandingStyles.titleText} numberOfLines={1}> · {title}</Text>
      </View>
    </View>
  );
}
...
function YourStandingCard({
  ...
  const {
    ...
    primaryNumPrefix,
    primaryLabel,
  } = getStandingCopy(state, rank, xp, nextRival, podiumTarget, leadOver);
  const progressPct = getChasePct(xp, nextRival, isThrone);
  const showChase = nextRival !== null || isThrone;

  return (
    <View style={[youStandingStyles.card, { borderColor: tier.borderActive, shadowColor: tier.color }]}>
      ...
      */}
      <View style={youStandingStyles.topRow}>
        <Reanimated.View
          entering={FadeInDown.delay(40).springify().damping(12)}
        >
...
function StoreView() {
  ...
  const [pendingPurchase, setPendingPurchase] =
    useState<CosmeticTheme | null>(null);
  ...
  return (
    <>
      ...
      >
        ...
        >
          ...
          <View style={storeStyles.walletInner}>
            ...
            <View style={storeStyles.walletBalanceRow}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={storeStyles.walletValue}>
                  {available.toLocaleString()}
                </Text>
                <Text style={storeStyles.walletValueLabel}>
                  COINS AVAILABLE
                </Text>
              </View>
            </View>
            ...
            <View style={storeStyles.walletLedger}>
              <View style={storeStyles.ledgerCell}>
                ...
                <Text style={storeStyles.ledgerValue}>
        ...
        <View style={storeStyles.section}>
          <View style={storeStyles.sectionHeader}>
            <View style={[leaderboardStyles.sectionDot, { backgroundColor: 'rgba(255,255,255,0.35)' }]} />
    ...
    >
      ...
      >
        ...
        >
          ...
          <Text style={[storeStyles.ctaText, { color: ctaColor }]}>
            {ctaLabel}
          </Text>
        </View>
      </Pressable>
        ...
        <Pressable style={confirmStyles.dismissArea} onPress={onClose}>
          <View style={confirmStyles.center} pointerEvents="box-none">
            ...
            >
              ...
              <Text
                style={[confirmStyles.rarityLabel, { color: r.color }]}
              >
                {r.label} THEME
              </Text>
              ...
              <View style={confirmStyles.balanceFrame}>
                ...
                <View style={confirmStyles.costPill}>
                  <Text style={confirmStyles.costText}>
                    −{theme.price.toLocaleString()}
                  </Text>
                  <Feather
                    name="arrow-right"
...
function LeaderboardView() {
  ...
  useFocusEffect(useCallback(() => { loadFollowed(); }, [loadFollowed]));
  useEffect(() => {
  ...
  const {
    ...
    const all = [
      ...others,
      { key: '__you__', name: 'You', title: displayTitle, xp: userXP, avatarUrl: profile?.avatar_url ?? null },
    ];
    ...
    const state: StandingState =
      me.rank === 1 ? 'throne' : me.rank <= 3 ? 'podium' : 'climbing';
    ...
    return {
      ...
      standingState: state,
      nextRival: above
        ? { name: above.name, xp: above.xp, rank: above.rank }
        : null,
      ...
      leadOver:
        me.rank === 1 && below
          ? { name: below.name, xp: below.xp, rank: below.rank }
          : null,
    };
  ...
  const buildPreview = useCallback(
    (entry: { key: string; name: string; rank: number; title: string; avatarUrl?: string | null }): ProfilePreviewData => {
...
function CurrentTierShimmer() {
  ...
  const style = useAnimatedStyle(() => ({
    ...
    opacity: interpolate(x.value, [-1, -0.5, 0, 0.5, 1], [0, 0.45, 0.8, 0.45, 0], Extrapolation.CLAMP),
  }));
  return (
...
function WalletShimmer() {
  ...
  const style = useAnimatedStyle(() => ({
    transform: [
      ...
      { translateY: interpolate(t.value, [-1, 1], [-WALLET_PANEL_H, WALLET_PANEL_H], Extrapolation.CLAMP) },
    ],
    opacity: interpolate(t.value, [-1, -0.5, 0, 0.5, 1], [0, 0.5, 0.85, 0.5, 0], Extrapolation.CLAMP),
  }));
  return (
    <Reanimated.View pointerEvents="none" style={[StyleSheet.absoluteFillObject, style]}>
      <LinearGradient
    ...
    >
      ...
      <View style={seasonStyles.bannerTop}>
        <View style={[seasonStyles.seasonBadge, { borderColor: currentRarity.borderActive, backgroundColor: currentRarity.color + '14' }]}>
          ...
          <Text style={[seasonStyles.seasonBadgeText, { color: currentRarity.color }]}>SEASON I</Text>
        </View>
        <View style={seasonStyles.bannerTopRight}>
          <View style={[seasonStyles.levelChip, { borderColor: accent + '66', backgroundColor: accent + '14' }]}>
...
function SeasonView({ equippedTitle, onEquip }: { equippedTitle: string | null; onEquip: (title: string | null) => void }) {
  return (
    <ScrollView style={{ flex: 1 }} contentContainerStyle={seasonStyles.scroll} showsVerticalScrollIndicator={false}>
      <SeasonBanner />
    ...
    >
      ...
      <Text
        style={[
          ...
          { color: isEquipped ? rarity.color : rarity.color + 'AA' },
        ]}
      >
...
const AchievementsScreen: React.FC = () => {
  ...
  const MUSCLE_GROUPS = ['Chest','Back','Shoulders','Hamstrings','Triceps','Biceps','Glutes','Quads','Calves','Core'] as const;
  ...
  const legSets       = (setsByMuscle.Quads ?? 0) + (setsByMuscle.Hamstrings ?? 0) + (setsByMuscle.Glutes ?? 0) + (setsByMuscle.Calves ?? 0);
  ...
  const balancedCount = totalWorkingSets > 0
    ? MUSCLE_GROUPS.filter(m => (setsByMuscle[m] ?? 0) / totalWorkingSets >= 0.10).length
    : 0;
  // Penalty-badge conditions (mirror-muscle = chest+biceps heavy / back+legs light)
  const visibleMuscleShare = totalWorkingSets > 0
    ? (chestSets + (setsByMuscle.Biceps ?? 0)) / totalWorkingSets
    : 0;
  const neglectedShare = totalWorkingSets > 0
    ? (backSets + legSets) / totalWorkingSets
    : 0;
  ...
  const achievements: Achievement[] = [
    ...
    { id: 'chest-100',         title: 'Chest Master',       description: '100 working sets on chest exercises',                  icon: 'target',      unlocked: chestSets     >= 100,    xp: 40,  progress: chestSets,     target: 100   },
    { id: 'glutes-100',        title: 'Glute Master',       description: '100 working sets on glute exercises',                  icon: 'octagon',     unlocked: (setsByMuscle.Glutes ?? 0) >= 100, xp: 40,  progress: setsByMuscle.Glutes ?? 0, target: 100 },
    { id: 'back-120',          title: 'Back Fortress',      description: '120 working sets on back exercises',                   icon: 'shield',      unlocked: backSets      >= 120,    xp: 45,  progress: backSets,      target: 120   },
    { id: 'legs-80',           title: 'Leg Day Devotee',    description: '80 working sets across quads, hamstrings, glutes & calves', icon: 'zap',         unlocked: legSets       >= 80,     xp: 45,  progress: legSets,       target: 80    },
    { id: 'shoulders-60',      title: 'Shoulder Garrison',  description: '60 working sets on shoulder exercises',                icon: 'trending-up', unlocked: shouldersSets >= 60,     xp: 75,  progress: shouldersSets, target: 60    },
    { id: 'arms-100',          title: 'Arm Sculptor',       description: '100 working sets across biceps & triceps',             icon: 'feather',     unlocked: armSets       >= 100,    xp: 75,  progress: armSets,       target: 100   },
    { id: 'core-50',           title: 'Core Fortress',      description: '50 working sets on core exercises',                    icon: 'anchor',      unlocked: coreSets      >= 50,     xp: 60,  progress: coreSets,      target: 50    },
    { id: 'chest-volume-50k',  title: 'Chest Volume Titan', description: 'Move 50,000 kg of chest volume',                       icon: 'layers',      unlocked: chestVolume   >= 50000,  xp: 125, progress: Math.round(chestVolume), target: 50000 },
    { id: 'vitruvian',         title: 'Vitruvian Lifter',   description: 'Reach 30+ working sets on every one of the 10 muscle groups', icon: 'award',       unlocked: vitruvianCount >= 10,    xp: 150, progress: vitruvianCount, target: 10    },
    { id: 'balanced',          title: 'Balanced Beast',     description: 'Every muscle group accounts for at least 10% of your total working sets', icon: 'grid',        unlocked: balancedCount >= 10,     xp: 175, progress: balancedCount, target: 10    },
    ...
    { id: 'chicken-legs',      title: 'Chicken Legs',           description: '100+ sessions logged with less than 5% of working sets on legs',                          icon: 'feather',  unlocked: totalSessions >= 100 && legShare < 0.05,                                            xp: 20, progress: totalSessions >= 100 && legShare < 0.05 ? 1 : 0, target: 1 },
    { id: 'glass-cannon',      title: 'Glass Cannon',           description: '50+ bench sets but fewer than 20 back sets — front-loaded power',                          icon: 'zap',      unlocked: benchSets     >= 50  && backSets   < 20,                                          xp: 25, progress: benchSets     >= 50  && backSets   < 20  ? 1 : 0, target: 1 },
    { id: 'mirror-muscle',     title: 'Mirror Muscle Society',  description: 'Chest + biceps make up 60%+ of your sets while back + legs sit under 20%',                icon: 'eye',      unlocked: totalWorkingSets > 0 && visibleMuscleShare >= 0.60 && neglectedShare < 0.20,         xp: 30, progress: totalWorkingSets > 0 && visibleMuscleShare >= 0.60 && neglectedShare < 0.20 ? 1 : 0, target: 1 },
    { id: 'cardio-phobic',     title: 'Cardio Phobic',          description: '50+ sessions logged with zero timed or distance sets',                                    icon: 'wind',     unlocked: totalSessions >= 50  && cardioSets === 0,                                         xp: 20, progress: totalSessions >= 50  && cardioSets === 0  ? 1 : 0, target: 1 },
    { id: 'core-neglect',      title: 'Forgotten Foundation',   description: '75+ sessions logged with fewer than 10 core sets',                                       icon: 'anchor',   unlocked: totalSessions >= 75  && coreSets   < 10,                                          xp: 30, progress: totalSessions >= 75  && coreSets   < 10  ? 1 : 0, target: 1 },

    // ── Bucket 3: grindy lifetime stretch goals ─────────────────────────────
    { id: '365-days',          title: 'A Full Year',            description: 'Train on 365 unique calendar days (not consecutive)',                                    icon: 'calendar', unlocked: uniqueDays      >= 365,       xp: 200, progress: uniqueDays,       target: 365       },
    { id: '100k-sets',         title: 'Hundred Thousand',       description: 'Complete 100,000 working sets',                                                          icon: 'layers',   unlocked: totalWorkingSets >= 100000,    xp: 175, progress: totalWorkingSets, target: 100000    },
    { id: 'blue-whale',        title: 'Lifted the Ocean',       description: '150,000 kg total volume — the weight of a blue whale',                                   icon: 'droplet',  unlocked: totalVolume      >= 150000,    xp: 175, progress: Math.round(totalVolume), target: 150000    },
    { id: 'boeing-747',        title: 'Lifted the Plane',       description: '412,775 kg total volume — the empty weight of a Boeing 747',                             icon: 'zap',      unlocked: totalVolume      >= 412775,    xp: 250, progress: Math.round(totalVolume), target: 412775    },
    { id: '1000-sessions',     title: 'Ironclad',               description: 'Complete 1,000 workout sessions',                                                        icon: 'star',     unlocked: totalSessions    >= 1000,      xp: 500, progress: totalSessions,    target: 1000      },
    { id: 'eternal-flame',     title: 'Eternal Flame',          description: '52 consecutive weeks with at least 2 sessions per week',                                 icon: 'sun',      unlocked: longestWeeklyRun >= 52,        xp: 500, progress: longestWeeklyRun, target: 52        },
    { id: '10m-volume',        title: 'Lifted the Whale',       description: 'Move 10,000,000 kg of lifetime volume',                                                 icon: 'hexagon',  unlocked: totalVolume      >= 10000000, xp: 600, progress: Math.round(totalVolume), target: 10000000  },
  ];
        ...
        <Animated.View style={{ flexDirection: 'row', width: SCREEN_WIDTH * TAB_COUNT, flexGrow: 1, alignSelf: 'stretch', transform: [{ translateX }] }}>
          ...
          <ScrollView style={{ flex: 1 }} contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
            ...
            >
              ...
              <View style={[styles.seasonDividerLine, { backgroundColor: accent + '59' }]} />
              <MaterialCommunityIcons name="star-four-points" size={10} color={accent} />
              <Text style={[styles.seasonDividerText, { color: accent }]}>LIFETIME ACHIEVEMENTS</Text>
              <MaterialCommunityIcons name="star-four-points" size={10} color={accent} />
              <View style={[styles.seasonDividerLine, { backgroundColor: accent + '59' }]} />
              <Feather
