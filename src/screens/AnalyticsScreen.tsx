...
import {
  loadAllSessions as sbLoadAllSessions,
  loadSessionsForWorkout,
  loadDurationsForWorkout,
...
function getExerciseModes(sessions: WorkoutSession[]): Record<string, AnalyticsMode> {
  ...
  for (const session of sessions) {
    for (const ex of session.exercises) {
      ...
      else if (working.some((s) => (s.minutes ?? 0) > 0 || (s.seconds ?? 0) > 0))
        modeMap[ex.name] = 'timed';
...
function computeSectionStats(sessions: WorkoutSession[], sectionName: string) {
  const present = sessions.filter((s) => s.exercises.some((e) => e.name === sectionName));
  const consistency =
    sessions.length > 0 ? Math.round((present.length / sessions.length) * 100) : 0;
...
const AnalyticsScreen: React.FC = () => {
  ...
  const lastPerformedLabel =
    sessions.length > 0 ? formatRelativeDate(new Date(sessions[sessions.length - 1].date)) : '—';
  const muscleSets = muscleSetsFromTemplate(workout);
  const hasMuscleSets = Object.keys(muscleSets).length > 0;
  const sessionExerciseNames = Array.from(
    new Set(
      sessions.flatMap((s) =>
  ...
  const bodyweightExercises = sessionExerciseNames.filter(
    (n) => exerciseModeMap[n] === 'bodyweight',
  );
                    ...
                    {cooldownStats && cooldownStats.count > 0 && (
                      <View style={styles.section}>
                        ...
                        <View style={styles.cardRow}>
                          <StatCard
                            label="Last"
                            value={cooldownStats.last > 0 ? fmtDuration(cooldownStats.last) : '—'}
                            icon="sunset"
                            compact
                          />
                          <StatCard
                            label="Avg"
                            value={cooldownStats.avg > 0 ? fmtDuration(cooldownStats.avg) : '—'}
                            icon="bar-chart-2"
                            compact
                          />
                          <StatCard
                            label="Consistency"
                            value={`${cooldownStats.consistency}%`}
                            icon="check-circle"
                            compact
                          />
                        </View>
            ...
            {visitedTabs.has(2) &&
              (allSessions.length === 0 ? (
                ...
                <ScrollView
                  ...
                  showsVerticalScrollIndicator={false}
                >
                  <OverallStats workouts={workouts} allSessions={allSessions} />
                  <View style={{ height: 40 }} />
                </ScrollView>
              ))}
          </View>
